...
export default class BossLevel extends Phaser.Scene {
  ...
  init() {
    ...
    this.countdownTimer = 390;
    this.timedEvent = undefined;
  }

  preload() {
    this.load.image("bosslevel", "images/levelBoss.jpg");
  ...
  create() {
    ...
    this.tileSprite = this.add
      .tileSprite(0, 0, 800, 450, "bosslevel")
      .setScale(1)
      .setOrigin(0, 0)
      .setScrollFactor(0, 0);
    this.levelText = this.add.text(350, 10, "BOSS LEVEL");
  ...
  spawnRobot(sprite, time) {
    ...
    if (robot.body) {
      const randomVelocity = Phaser.Math.Between(-150, -250);
  ...
  createPortal() {
    const x = Phaser.Math.Between(30000, 40000);
